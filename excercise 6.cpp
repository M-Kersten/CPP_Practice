#include <iostream>
// library voor dynamisch data inladen en random nummer generation
#include <cstdlib>

using namespace std;

// functie om dynamisch een array in te laden
int * createArray(int);

// functie die de waardes locaal omdraait en daarna via pointers de waardes van de variabelen aanpast.
void swapTwoValues(int*, int*);

void fillArrayWithValues(int[], int);
void printArrayValues(int[], int);
//sinking sort of bubble sort is de meest simpele sorteermethode die waardes naast elkaar vergelijkt en ze op de juiste plek zet.
//https://www.go4expert.com/articles/bubble-sort-algorithm-absolute-beginners-t27883/
void bubbleSort(int[], int, int = 1);
// selection sort is een sorteermethode die door een serie waardes heen loopt en ze stuk voor stuk op de juiste plek zet.
// https://www.tutorialspoint.com/data_structures_algorithms/selection_sort_algorithm
void selectionSort(int[], int, int = 1);
// wachten met het sluiten van het programma doe ik zo, is niet de beste manier maar werkt voor visual studio
int waitForQuit;
...
void swapTwoValues(int* firstValue, int* secondValue)
{
	...
	// simpele omkering van waardes dmv locale variabele
	temporaryVariable = *firstValue;
	*firstValue = *secondValue;
	*secondValue = temporaryVariable;
}

// functie om handmatig array elementen in te voeren
...
void selectionSort(int arrayToSort[], int sizeOfArray, int orderOfSequence)
{
	...
	for (int i = 0; i < sizeOfArray - 1; i++)
	{
		...
		{
			//zoek naar nabijgelegen waardes
