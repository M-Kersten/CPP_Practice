#include <iostream>
// een constante waarde vaststellen
#define PI 3.14
using namespace std;

//functies moet je aangeven voordat je ze gebruikt
void initMenu();
...
int main()
{
    ...
    do
    {
        ...
        // is de waarde verkeerd?(dit is een int, geen char en dus moet ik elke input apart doorlopen)
        while(!(cin >> choice))
        {
            //cout << "state before: " << cin.rdstate() << endl;
            cin.clear();
            //cout << "state after: " << cin.rdstate() << endl;
            //negeer alles, de maximale lengte van de filestream (iostream) dit kan handig zijn als er lange stukken tekst moet worden gechekt.
            cin.ignore(numeric_limits<streamsize>::max(), '\n');

            system("cls");
            initMenu();
            cout << "You typed the wrong data to the input. " << endl;
        }

        //keuze doorgeven aan functie
        menuDecision(choice);
...
void menuDecision(int choice)
{
    ...
    {
        case 1:
            // waarde doorsturen naar isvalid om te checken of er een geldige input is gegeven.
